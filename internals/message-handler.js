...
const iterate = async(client) => {
    for await(const talent of talentSchema.talent.find(!{guildID: '838711689125822474'})){
        for await(var i of talent.upcomingStreams){
            let curDate = new Date(talent.upcomingStreams[i].startTime)
            if(curDate.setMinutes(curDate.getMinutes()-15) < new Date()){
                await (await client.channels.cache.get(talent.liveChannelID)).send(`Hey ${talent.roleID}! ${talent.name} is streaming in 15 minutes! Feel free to join us at https://www.youtube.com/watch?v=${talent.upcomingStreams[i].videoID}`)
